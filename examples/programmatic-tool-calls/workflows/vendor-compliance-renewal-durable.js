...
Capabilities:
  ...
  - checkpoint_vendor_review(reviewDraft) -> { approved, reviewer, reason }
  - file_vendor_review(payload) -> { reviewRecordId, state }
*/

async function runVendorComplianceRenewalDurable() {
  ...
  const approval = await checkpoint_vendor_review({
    vendorId,
    reviewCycleId,
    requiredFrameworks,
    vendor,
...
runVendorComplianceRenewalDurable();
