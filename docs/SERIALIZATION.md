...
- Public `ExecutionSnapshot` serde deserialization reuses the same validation,
  accounting recomputation, cached post-load accounting state, and
  explicit-policy gate as `load_snapshot()`, so callers cannot bypass restore
  checks by deserializing the public type directly.
