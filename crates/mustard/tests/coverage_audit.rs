    ...
    match stmt {
        ...
        Stmt::For {
            ...
            init.as_ref().is_some_and(|init| match init {
                mustard::ir::ForInit::VariableDecl { declarators, .. } => declarators
                    .iter()
                    .filter_map(|declarator| declarator.initializer.as_ref())
                    .any(|initializer| expr_contains(initializer, predicate)),
                mustard::ir::ForInit::Expression(expression) => {
                    expr_contains(expression, predicate)
                }
            }) || test
                .as_ref()
        ...
        Stmt::ForIn { object, body, .. } => {
            expr_contains(object, predicate) || stmt_contains_expr(body, predicate)
        }
...
fn ir_covers_supported_destructuring_and_short_circuit_forms() {
    ...
    assert!(matches!(
        ...
        Stmt::VariableDecl { declarators, .. }
            if matches!(declarators[0].pattern, mustard::ir::Pattern::Object { .. })
    ));
    assert!(matches!(
        &program.script.body[1],
        Stmt::VariableDecl { declarators, .. }
            if matches!(declarators[0].pattern, mustard::ir::Pattern::Array { .. })
    ));
...
fn ir_covers_sequence_and_exponentiation_expressions() {
    ...
    assert!(stmt_contains_expr(
        ...
        &|expr| matches!(
            expr,
            Expr::Binary {
                operator: mustard::ir::BinaryOp::Pow,
                ..
            }
        )
    ));
...
fn ir_covers_in_operator_expressions() {
    ...
    assert!(stmt_contains_expr(
        ...
        &|expr| matches!(
            expr,
            Expr::Binary {
                operator: mustard::ir::BinaryOp::In,
                ..
            }
        )
    ));
