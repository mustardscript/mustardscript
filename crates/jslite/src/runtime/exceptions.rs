...
impl Runtime {
    ...
    pub(super) fn annotate_runtime_error(&self, error: JsliteError) -> JsliteError {
    ...
    pub(super) fn handle_runtime_fault(&mut self, error: JsliteError) -> JsliteResult<StepAction> {
        match error {
            JsliteError::Message {
                kind: DiagnosticKind::Runtime,
                message,
    ...
    pub(super) fn complete_return(&mut self, value: Value) -> JsliteResult<StepAction> {
        let frame = self
            .frames
            .pop()
            .ok_or_else(|| JsliteError::runtime("vm lost all frames"))?;
