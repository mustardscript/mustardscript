...
- [x] Finish reducing promise-outcome clone amplification in settlement,
...
- [x] Replace JSON-string hot paths for addon start/resume traffic with a typed
  or binary boundary path.
- [x] Reduce JS-side structured DTO creation on hot host-boundary paths.
...
- [x] Keep the Node wrapper thin and keep all semantic validation in Rust.
- [x] Check in a fresh representative artifact showing the boundary win on the
...
| 2026-04-14T03:44:00Z | Committed as `9615e2b`, replaced the live addon start/resume JSON hot path with a binary buffer transport, removed JS structured-DTO construction from the live addon path while preserving fail-closed boundary checks, refreshed the representative artifact plus findings/website export, and closed the remaining Milestone 1 async-clone item by auditing the current keyed promise queueing against the Rust bench suite. | Updated `crates/mustard-node/src/boundary_binary.rs`, `crates/mustard-node/src/lib.rs`, `lib/structured.ts`, `lib/runtime.ts`, `lib/progress.ts`, `tests/node/addon-boundary-profile.test.js`, `tests/node/execution-context.test.js`, `benchmarks/workloads.ts`, `docs/BENCHMARK_FINDINGS.md`, `website/src/generated/benchmarkData.ts`, `index.d.ts`, and `benchmarks/results/2026-04-14T03-41-06-633Z-workloads.json`. Verified `npm run bench:rust`, `npm run bench:workloads:release`, `cargo test --workspace`, `npm test`, `npm run lint`, and `npm run test:use-cases`. Relative to `2026-04-14T03-01-24-879Z`, the kept artifact moved `addon.ptc.weightedScore.medium 0.71 ms -> 0.66 ms` (`-7.0%`), `ptc_website_demo_small 0.15 ms -> 0.13 ms`, `ptc_fraud_investigation_medium 1.46 ms -> 1.33 ms`, `ptc_vendor_review_medium 0.22 ms -> 0.19 ms`, and cut addon boundary codec time on the representative lanes (`website 0.012 ms -> 0.007 ms`, `incident 0.036 ms -> 0.021 ms`, `fraud 0.183 ms -> 0.078 ms`, `vendor 0.026 ms -> 0.012 ms`) while fraud guest execution stayed flat (`0.831 ms -> 0.827 ms`). The current Rust async benches (`promise_all_immediate_fanout ~0.38 ms`, `promise_all_settled_immediate ~3.39 ms`, `promise_all_derived_ids_fanout ~6.21 ms`, `promise_all_map_set_reduction ~8.44 ms`) remain well below the dominant synthetic local hotspots (`token_normalize ~70 ms`, `top_k_sort ~208 ms`), so the remaining async-clone checkbox no longer reflects an unresolved representative bottleneck. | No external blocker remains. The representative PTC plan is complete at `9615e2b`. |
