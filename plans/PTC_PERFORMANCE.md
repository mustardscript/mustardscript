...
- [ ] Add per-lane boundary breakdowns showing:
...
- [ ] Use the new PTC lanes to decide whether string/key interning should be
  prioritized next.
- [ ] Audit real example-inspired lanes for avoidable temporary allocation and
  cloning.
- [ ] Re-run keyed-collection, builtin, async-runtime, and use-case tests after
...
- [ ] Add a durable-boundary variant of the workflow lane where the final
  writeback occurs after a persisted checkpoint.
- [ ] Record snapshot bytes, detached manifest bytes, and resume-only timing for
  that lane.
- [ ] Keep the final-action semantics and failure behavior aligned across addon
  and sidecar.
- [ ] Preserve the current suspend/resume advantage over isolates on that lane.
