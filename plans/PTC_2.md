...
- [x] Improve at least two lanes that spend real time in string cleanup.
- [x] Keep holdout lanes with different text distributions flat or better.
- [x] Preserve exact result stability across nominal and skewed datasets.
...
- [x] Add program-counter-local feedback for hot property, collection, and
  string sites.
- [x] Back-patch eligible bytecodes to specialized handlers after a guarded warm
  threshold.
- [x] Support invalidation when the observed shape becomes too polymorphic or
  when the specialization assumptions stop holding.
- [x] Report patched-site count, hit rate, invalidation count, and deopt count.
- [x] Start with a narrow audited set of specializations instead of a large
  speculative matrix.
- [x] Require every new specialized handler to prove benefit on the broad panel
...
| 2026-04-14T13:16:06Z | Stabilized the phase-2 release measurement profile by batching warm samples for `ptc_public`, headline, broad, holdout, and sentinel modes, checked in new kept broad/holdout artifacts, and fixed parallel Rust test flakes by serializing the fast-path override guards used by the runtime/compiler/string lookup tests. Under the stable profile the current default-on string fast paths now satisfy the Milestone 8 success checks, so that milestone closes. | Commits `13240f9` and `9206fb5`; verified `node --test tests/node/benchmark-support.test.js tests/node/benchmark-compare.test.js`, `npm run bench:ptc:broad` (twice), `npm run bench:ptc:holdout`, `node scripts/benchmark-compare.ts --baseline benchmarks/results/2026-04-14T12-23-45-749Z-ptc_broad_release-release.json --candidate benchmarks/results/2026-04-14T13-05-10-833Z-ptc_broad_release-release.json --include-prefix addon.ptc.phase2.scorecards.headlineScore.medium --include-prefix addon.ptc.phase2.scorecards.broadScore.medium --include-prefix addon.ptc.phase2.scorecards.p90LaneRatio.medium --include-prefix addon.ptc.phase2.scorecards.worstLaneRatio.medium --require-path addon.ptc.phase2.scorecards.headlineScore.medium --require-path addon.ptc.phase2.scorecards.broadScore.medium --require-path addon.ptc.phase2.scorecards.p90LaneRatio.medium --require-path addon.ptc.phase2.scorecards.worstLaneRatio.medium`, `node scripts/benchmark-compare.ts --baseline benchmarks/results/2026-04-14T12-24-04-949Z-ptc_holdout_release-release.json --candidate benchmarks/results/2026-04-14T13-05-19-118Z-ptc_holdout_release-release.json --include-prefix addon.ptc.phase2.scorecards.holdoutScore.medium --require-path addon.ptc.phase2.scorecards.holdoutScore.medium`, `cargo test --workspace`, `npm test`, `npm run lint`, and `npm run test:use-cases`; kept artifacts `benchmarks/results/2026-04-14T13-05-10-833Z-ptc_broad_release-release.json` and `benchmarks/results/2026-04-14T13-05-19-118Z-ptc_holdout_release-release.json`, where broad moved `0.58 ms -> 0.47 ms`, headline `0.69 ms -> 0.51 ms`, holdout `0.63 ms -> 0.47 ms`, `p90` ratio `1.04x -> 0.92x`, and worst-lane `p95` stayed within guardrails at `1.37x -> 1.41x` | None; the benchmark-instability blocker is resolved, so the next feasible path is Milestone 9 feedback-directed specialization while Milestone 7 collection fast paths remain opt-in experiments. |
| 2026-04-14T13:38:39Z | Implemented the first Milestone 9 slice as a guarded, feedback-directed property specialization pass: program-counter-local feedback now tracks hot property, collection, and string sites; monomorphic shape-backed property sites can back-patch into a runtime-local fast path after a warm threshold; invalidations/deopts disable unstable sites; and representative benchmark output now reports feedback hot-site and patch hit/fallback counters. The broad-panel benchmark candidate did not beat the kept baseline, so the handler remains opt-in behind `MUSTARD_ENABLE_FEEDBACK_PROPERTY_PATCHING` instead of shipping by default. | Commit `86e43d1`; verified `cargo test -p mustard --lib feedback_ -- --nocapture`, `cargo test -p mustard --test runtime_debug_metrics`, `node --test tests/node/benchmark-support.test.js tests/node/benchmark-compare.test.js tests/node/ptc-attribution.test.js tests/node/ptc-benchmarks.test.js`, `cargo test --workspace`, `npm test`, `npm run lint`, and `npm run test:use-cases`; benchmarked `MUSTARD_ENABLE_FEEDBACK_PROPERTY_PATCHING=1 npm run bench:ptc:broad` and compared `benchmarks/results/2026-04-14T13-32-05-159Z-ptc_broad_release-release.json` against the kept broad artifact `benchmarks/results/2026-04-14T13-05-10-833Z-ptc_broad_release-release.json`, which moved headline `0.51 ms -> 0.52 ms`, broad `0.47 ms -> 0.47 ms` (`+0.7%`), `p90` ratio `0.92x -> 0.96x`, and worst-lane `p95` `1.41x -> 1.59x` | Broad-panel evidence still blocks default-on rollout of the new handler: the candidate does not improve headline and broad together, and the worst-lane tail regresses materially versus the kept broad artifact, so Milestone 9 stays open while the implementation remains an opt-in experiment. |
| 2026-04-14T13:47:22Z | Tuned the opt-in feedback patch experiment by lowering the warm threshold from `16` to `8`, which is enough for audited shape-backed row sites to patch during representative broad-panel runs instead of only in synthetic test fixtures. The tuned candidate now shows an actual patched property site hit on `ptc_vendor-compliance-renewal_medium` and improves the broad-panel median slightly, but it still does not clear the headline-plus-broad success bar for default-on rollout. | Commit `106ba79`; re-verified `cargo test -p mustard --lib feedback_ -- --nocapture`, `cargo test -p mustard --test runtime_debug_metrics`, `cargo test --workspace`, `npm test`, `npm run lint`, and `npm run test:use-cases`; benchmarked `MUSTARD_ENABLE_FEEDBACK_PROPERTY_PATCHING=1 npm run bench:ptc:broad` and compared `benchmarks/results/2026-04-14T13-41-12-362Z-ptc_broad_release-release.json` against the kept broad artifact `benchmarks/results/2026-04-14T13-05-10-833Z-ptc_broad_release-release.json`, which moved broad `0.47 ms -> 0.46 ms`, headline `0.51 ms -> 0.52 ms`, `p90` ratio median `0.92x -> 0.92x`, and worst-lane `p95` `1.41x -> 1.44x`; the representative addon counters in the new candidate artifact now show `feedback patch sites 1` and `feedback patch hit/fallback/invalidation/deopt 1/0/0/0` on `ptc_vendor-compliance-renewal_medium` | The tuned handler is a better opt-in experiment than the original slice, but it still does not improve headline and broad together and therefore still fails the plan’s evidence bar for default-on rollout; Milestone 9 remains open. |
