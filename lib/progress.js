...
function assertSnapshotNotUsed(native, snapshotTokenValue) {
  if (callNative(native.isProgressSnapshotUsed, snapshotTokenValue)) {
...
function createProgressApi(native) {
  class Progress {
    ...
    #claimSnapshot() {
      if (!callNative(native.claimProgressSnapshot, this.#snapshotToken)) {
        throw new JsliteError(
          'Runtime',
          'Progress objects are single-use; this suspended execution was already resumed',
        );
    ...
    static load(state, options = undefined) {
      ...
      assertSnapshotNotUsed(native, state.token);
