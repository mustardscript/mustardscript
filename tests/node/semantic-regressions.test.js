...
test('loose equality performs abstract equality in ordinary and lenient programs', async () => {
  const source = '[undefined == null, "1" == 1, 1 != "1", undefined === null, "1" === 1];';
  await assertDifferential(source);
  for (const lenientMode of [false, true]) {
    Mustard.validateProgram(source, {lenientMode});
    assert.deepEqual(await Mustard.load(runtime(source, {lenientMode}).dump()).run(), [true, true, false, false, false]);
  }
});
